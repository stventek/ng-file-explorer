...
export function calculateContextMenuPosition(event: MouseEvent, contextMenuRef: ElementRef) {
    ...
    if (availableSpaceBottom < contextMenuHeight && availableSpaceTop >= contextMenuHeight) {
      position = 'top-right';
    }

    if (availableSpaceRight < contextMenuWidth && availableSpaceLeft >= contextMenuWidth) {
